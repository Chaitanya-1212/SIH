...
export interface AppState {
  ...
  addBeneficiary: (newBeneficiary: Beneficiary) => void;
...
const getInitialLanguage = (): Language => {
  if (typeof window !== 'undefined') {
    try {
      const saved = localStorage.getItem('sanjeevani_language');
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  speakVernacular: async (text) => {
    ...
    const prompt =
      text ||
      `${beneficiary.name}, रक्तदाब ${beneficiary.vitals.systolic}/${beneficiary.vitals.diastolic} mmHg.`;
  ...
  addBeneficiary: (newBeneficiary) => {
    watermelonDb.addBeneficiary(newBeneficiary);
    set({
  ...
  triggerStoreAndForwardSync: async () => {
    ...
    try {
      ...
      set({
        isSyncing: false,
        syncQueue: watermelonDb.getSyncQueue(),
        syncSuccessMessage: `✓ ${result.syncedCount} नोंदी केंद्रीय सर्व्हरवर यशस्वीरित्या सिंक झाल्या (Tx: ${result.postgresTxId})`,
      });
      setTimeout(() => {
  ...
  loginWithJwt: async (workerId, pin) => {
    try {
      ...
      if (session) {
        set({ jwtSession: session, currentScreen: 'home' });
        return true;
      }
      return false;
    } catch {

...
export class WatermelonDbEngine {
  ...
  private initDatabase(): void {
    if (!localStorage.getItem(WATERMELON_DB_KEY)) {
  ...
  public getBeneficiaries(): Beneficiary[] {
    try {
      ...
      return parsed.tables.beneficiaries;
    } catch {
  ...
  public updateBeneficiaryVitals(
    ...
    const isHighRisk = vitals.systolic >= 140 || vitals.diastolic >= 90;

    if (idx !== -1) {
      db.tables.beneficiaries[idx].vitals = vitals;
      if (isHighRisk) {
        db.tables.beneficiaries[idx].riskStatus = 'HIGH';
        db.tables.beneficiaries[idx].riskTitle = 'Gestational Hypertension (उच्च रक्तचाप)';
      }
